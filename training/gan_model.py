...
class GANModel(nn.Module):
    def __init__(self, opt):
        ...
        self.device = 'cpu'
        # seems this module never actually used
    ...
    def execute(self, data, mode):
        ...
        if mode == 'generator':
            ...
            print('compute discriminator regularize')
    ...
    def create_optimizers(self, opt):
        ...
        if opt.optim_param_g == 'style':
            ...
            raise ValueError(
                "--optim_param_g tag should be 'style' or 'w_shift', but get ", opt.optim_param_g)
        ...
        d_reg_ratio = opt.d_reg_every / (opt.d_reg_every +
                                         1) if not opt.no_d_regularize else 1.
        ...
        optimizer_G = jt.optim.Adam(
            G_params, lr=G_lr, betas=(G_beta1, G_beta2))
        optimizer_D = jt.optim.Adam(
            D_params, lr=D_lr, betas=(D_beta1, D_beta2))

        return optimizer_G, optimizer_D

    def create_loss_fns(self, opt):
        self.criterionGAN = networks.GANLoss(
            opt.gan_mode, tensor=None, opt=self.opt)
        if opt.l_weight > 0:
    ...
    def inference(self, noise, trunc_psi=1.0, mean_latent=None, with_tf=False):
        ...
        img = self.netG([noise], truncation=trunc_psi,
    ...
    def compute_generator_loss(self):
        ...
        if self.opt.l_weight > 0:
            ...
            G_losses['l_weight'] = self.opt.l_weight * \
                self.weight_loss(self.G_params)
    ...
    def compute_discriminator_regularization(self, real_sketch, real_image):
        ...
        D_reg_losses['D_r1_loss_sketch'] = self.opt.r1 / \
            2 * r1_loss * self.opt.d_reg_every
    ...
    def generate_fake(self, batch_size=None, style_mix=True, return_latents=False):
        ...
        noises = mixing_noise(batch_size, self.opt.z_dim,
                              style_mix_prob, device)
