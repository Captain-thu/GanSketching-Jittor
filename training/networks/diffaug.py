...
def rand_translation(x, ratio=0.125):
    ...
    grid_batch, grid_x, grid_y = jt.meshgrid(
        jt.arange(x.size(0), dtype=jt.int32),
        jt.arange(x.size(2), dtype=jt.int32),
        jt.arange(x.size(3), dtype=jt.int32),
    )
...
def rand_upscale(x, ratio=0.25):
    ...
    sz = x.size(2)
    x = nn.functional.interpolate(x, scale_factor=up_ratio, mode='bilinear')
