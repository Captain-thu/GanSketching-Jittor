...
def weights_init_normal(m):
    ...
    if classname.find('Conv') != -1:
        ...
        init.gauss_(m.weight.data, 1.0, 0.02)
...
# not implemented by jittor and seems not to be used, so deprecated
...
# seems this function never be used
...
class GANLoss(nn.Module):
    ...
    def get_target_tensor(self, input, target_is_real):
        ...
        if target_is_real:
            ...
            if create_label:
                self.real_label_var = jt.full(
                    input.size(), self.real_label, requires_grad=False)
            target_tensor = self.real_label_var
        else:
            create_label = ((self.fake_label_var is None) or
                            (self.fake_label_var.numel() != input.numel()))
            if create_label:
                self.fake_label_var = jt.full(
                    input.size(), self.fake_label, requires_grad=False)
...
class GlobalDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False):
        ...
        sequence += [nn.Conv2d(ndf * nf_mult, 1,
                               kernel_size=kw, stride=2, padding=0)]
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False):
        ...
        sequence = [
            nn.Conv2d(input_nc, ndf,
                      kernel_size=kw[0], stride=2, padding=padw),
            nn.LeakyReLU(0.2, True)
        ]
        # 125
        nf_mult = 1
        ...
        # 6
        sequence += [nn.Conv2d(1, 1, kernel_size=6,
