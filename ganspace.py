...
def apply_shift(g, mean_latent, latents, w_comp, w_std, s, layers, w_plus=False, trunc=0.5):
    ...
    latents[:, layers, :] = latents[:, layers, :] + \
        w_comp[:, None, :] * s * w_std

    im = g([latents], input_is_latent=True, truncation=trunc,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--obj', type=str, choices=['cat', 'horse', 'church'], help='which StyleGAN2 class to use')
    parser.add_argument('--comp_id', type=int, required=True,
                        help="which principle component to use")
    parser.add_argument('--scalar', type=float, required=True,
    ...
    with jt.no_grad():
        ...
        # generate outputs
        ims = apply_shift(netG, mean_latent, latents, w_comp,
                          w_std, 0, layers, trunc=args.truncation)
        save_ims(f'./{args.save_dir}/before_', ims)
        ims = apply_shift(netG, mean_latent, latents, w_comp,
                          w_std, s, layers, trunc=args.truncation)
