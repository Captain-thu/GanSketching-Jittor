...
from utils import init_tf, initialize_feature_extractor
import argparse
...
def compute_precision_recall(path_real, path_fake, minibatch_size, num_images,
    ...
    ref_features = get_features(
        path_real, feature_net, num_images, minibatch_size, num_gpus=num_gpus)

    # Calculate VGG-16 features for generated images.
    print('Generating images...')
    eval_features = get_features(
        path_fake, feature_net, num_images, minibatch_size, num_gpus=num_gpus)

    # Calculate k-NN precision and recall.
    state = knn_precision_recall_features(
        ref_features, eval_features, num_gpus=num_gpus)
...
def get_features(images, feature_net, num_images, minibatch_size, num_gpus=1):
    if type(images) == str:
        ...
        raise ValueError(
            f"type of images should be list or string, but got {type(images)}")

    assert num_images == input_images.shape[0]
    features = np.zeros(
        [num_images, feature_net.output_shape[1]], dtype=np.float32)

    for begin in range(0, num_images, minibatch_size):
        end = min(begin + minibatch_size, num_images)
        real_batch = input_images[begin:end]
        features[begin:end] = feature_net.run(
            real_batch, num_gpus=num_gpus, assume_frozen=True)
