...
class Evaluator():
    def __init__(self, opt, gan_model):
        ...
        # with mp.Pool(1) as p:
        #     self.fid_stat = p.apply(
        #         get_fid_stats, (opt.eval_dir, opt.eval_batch))
...
def metrics_process(cache_folder, fid_stats, vgg_feats):
    ...
    metrics['fid'] = fid.compute_fid(cache_folder+'/image/', dataset_name=fid_stats, dataset_split="custom")
...
def get_fid_stats(eval_dir, eval_batch):
    ...
    if not fid.test_stats_exists(fid_stat, 'clean'):
        fid.make_custom_stats(fid_stat, eval_dir +
                              '/image/', batch_size=eval_batch)
