...
def make_eval_images(g, save_folder, eval_samples, batch_size, device, to_cpu=True):
    ...
    for i in tqdm(range(iterations)):
        ...
        for ind in range(sample.size(0)):
            misc.save_image(
                sample[ind], f'{save_folder}/image/{str(img_count).zfill(6)}.png', 1, 8, True, (-1, 1))
            img_count += 1
    gc.collect()
    jt.gc()
...
def get_metrics(opt, name, target):
    ...
    ppl_wend = compute_ppl(g, num_samples=50000, epsilon=1e-4, space='w',
                           sampling='end', crop=False, batch_size=25, device='cuda')
    del g
    # torch.cuda.empty_cache()

    fake_feats, real_feats = stats_fake['vgg_features'], stats_real['vgg_features']
    # todo: do not use multi processing
    # with mp.Pool(1) as p:
    #     precision, recall = p.apply(
    #         run_precision_recall, (real_feats, fake_feats))
...
def get_stats(opt, g, folder):
    ...
    if not file_cached:
        make_eval_images(g, folder, opt.eval_samples,
...
def setup_generator(ckpt_path, w_shift=False):
    ...
    # todo: may not need to translate torch to jittor, this file is used to eval
...
if __name__ == '__main__':
    ...
    # todo: the rest of this file has no grad??
    ...
    with jt.no_grad():
        ...
        for name, target in zip(all_models, all_targets):
            ...
            table = pd.DataFrame.from_dict(
                metrics, orient='index', columns=table_columns)
